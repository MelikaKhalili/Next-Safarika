...
interface ProductGuideCustomProps {
  products: Product[];
  gridRows?: number;
}

export default function ProductGuideCustom({
  products,
  gridRows,
}: ProductGuideCustomProps) {
  return (
    <Swiper
      modules={[Navigation]}
      spaceBetween={20}
        ...
        return (
          <SwiperSlide key={index}>
            <Link href={`/products/${product.id}`} passHref>
              <div
                key={index}
                className="w-full h-[400px] rounded-xl hover:shadow-2xl overflow-hidden flex flex-col !bg-green-600"
              >
                ...
                <div className="flex flex-col p-4 text-white gap-4 !p-6 flex-grow">
                  <h2 className="!text-[17px] font-bold">{product.TourName}</h2>
                  <div className="flex w-full justify-between">
                    <p>{product.Los || "مدت زمان تور"}</p>
                    <p>
                      مجری تور :
                      <p className="inline-block">{product.Pres || "نامشخص"}</p>
                    </p>
                  </div>
                  <div className="flex w-full justify-between">
                    <p>{product.Price || "0"} تومان</p>
                  ...
                  <div className="flex w-full justify-between mt-auto">
                    <div className="flex items-center justify-center">
                      ...
                      <p>{product.Moveday || "تاریخ حرکت"}</p>
                    </div>
                    <svg
                      viewBox="0 0 24 24"
                      fill="none"
