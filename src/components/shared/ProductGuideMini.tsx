...
interface ProductGuideMiniProps {
  products: Product[];
  gridRows?: number;
}

export default function ProductGuideMini({
  products,
  gridRows,
}: ProductGuideMiniProps) {
  return (
    <Swiper
      modules={[Navigation]}
      spaceBetween={15}
        ...
        return (
          <SwiperSlide key={index}>
            <Link href={`/products/${product.id}`} passHref>
              <div
                key={index}
                className="w-[16rem] h-72 bg-red-600 rounded-xl hover:shadow-2xl overflow-hidden flex flex-col mb-40"
              >
                ...
                <div className="flex flex-col p-3 text-white !bg-amber-600 gap-2">
                  <h2 className="!text-[14px] font-bold">{product.TourName}</h2>
